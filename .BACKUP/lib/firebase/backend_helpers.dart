...
class UserFromFirebase {
  ...
  Future getCurrentUser() async {
    return userRef.doc(FirebaseAuth.instance.currentUser!.uid).get()
        .then((querySnap) {
...
Stream<dynamic> commentStream(Stream<DocumentSnapshot<Map<String, dynamic>>> source) async* {
  try {
    await for (final data in source) {
      List<DocumentReference> discussionList = [];
      dynamic comments = data.data()?['discussion'];
      
      for (final comment in comments) {
        discussionList.add(comment);
      }

      yield List.from(discussionList.reversed);
    }
  } catch (err) {
    yield [];
  }
...
Future<String> firebaseStorageToURL(ref) {
...
Map<String, dynamic> addComment(comment, uid, postReference) {
